...
from Preparedata import PU_csv
...
class ModelBase(nn.Module):
    ...
    def __init__(self, dim=128) -> None:
        ...
        self.net = ResNet1D.resnet18(norm_layer=None)      #encoder is a resnet18
...
class TFPrediction(nn.Module):
    def __init__(self, dim=128) -> None:
        super().__init__()
        self.encoderT = ModelBase()         # time domain encoder 
        self.encoderF = ModelBase()         # freq domain encoder
    ...
    def forward(self, x_t, x_f):
        ...
        return x_t, x_f        # shape of x_t and x_f (batch_size, 128)
...
def main_evaluate(args):
    ...
    model = ModelBase(dim=classes).to(device)       # defines a new encoder with the dim as the no of classes 
    checkpoint = torch.load("./History/TFPred_checkpoint.pth", map_location="cpu")
    
    '''Extracting the weights from onyl the encoder (encoderT) part of the model'''
    
    for k in list(checkpoint.keys()):
        # retain only encoder up to before the embedding layer
        if k.startswith('encoderT'):       #if the weights are for the encoderT
            # remove prefix
            checkpoint[k[len("encoderT."):]] = checkpoint[k]  #renames or creates a new key the keys from encoderT.conv1.weight to conv1.weight
        # delete renamed or unused k
        del checkpoint[k]   #deletes the old keys 
    #delets all the wieghts of fully connected layers 
    for k in list(checkpoint.keys()):
        if k.startswith('fc'):
            del checkpoint[k]
            
     #loads the weights from the checkpoints         
    missing_keys, unexpected_keys = model.load_state_dict(checkpoint, strict=False)  
    
    #makes sure taht the missing keys are from the fc layer cuz we delted it.
    assert missing_keys == ["fc.weight", "fc.bias"]
    
    #initializing a new classification layer
    model.fc.weight.data.normal_(mean=0.0, std=0.1)
    model.fc.bias.data.zero_()

    #store the parameters of the fc and the encoder seperately 
...
def train(args, model, train_loader, criterion, optimizer, device):
    ...
    with tqdm(total=len(train_loader), ncols=70, leave=False) as pbar:
        for i, (x_t, x_f, _) in enumerate(train_loader):
            ...
            x_t, x_f = model(x_t, x_f)      #passign the x_t and x_f into the TFPrecition class 
...
def main(args):
    ...
    # lr_scheduler
    lr_scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, args.max_epochs)        #the learning rate starts at a max value and then decreases following the shape of a cosine curve 

    # loss
    criterion = CrossCorrelationLoss()     #setting up the cross correaltion loss as the citeria 
    
    logging.info(">>>>> TFPred Pre-training ...")
    best_loss = 1e9
    for epoch in range(args.max_epochs):
        train_loss = train(args, model, train_loader, criterion, optimizer, device)  #calls the train fucn 

        # update lr
        if lr_scheduler is not None:
            lr_scheduler.step()
        
        if train_loss < best_loss:     #if the tranning loss is min so far, tag it as best loss and save the model weights 
